/**
 * Settings Registry — the single source of truth for the closed set of
 * application settings (key names, groups, value types and defaults).
 *
 * The entries mirror the currently-hardcoded behavior knobs cataloged in:
 * - `docs/BEHAVIOR-CONTROL-MASTER-LIST.md`
 * - `docs/APPLICATION-BEHAVIOR-CONTROL-RESEARCH.md` (§4 / §5 / §9.3)
 *
 * Defaults are copied verbatim from the current hardcoded values (uploads
 * constants, analyze route, learning rule, player limits, frontend branding…).
 *
 * Other packages import `SETTINGS_REGISTRY` (and `SettingKey`) to validate
 * keys/defaults; the seed script (`src/seed-settings.ts`) materializes this
 * registry into the `app_settings` table.
 */
...
/**
 * The closed union of all valid setting keys (NOT freeform). Derived from the
 * registry so it can never drift from the list above.
 */
