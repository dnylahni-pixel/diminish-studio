/**
 * Generated by orval v8.9.1 🍺
 * Do not edit manually.
 * Api
 * DiminishStudio API - Music learning and chord analysis platform
 * OpenAPI spec version: 0.1.0
 */
import * as zod from 'zod';


/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  "status": zod.string()
})


/**
 * @summary Check runtime dependencies
 */
export const ReadinessCheckResponse = zod.object({
  "status": zod.enum(['ok', 'error']),
  "dependencies": zod.object({
  "database": zod.enum(['ok', 'error'])
})
})


/**
 * Returns the in-memory runtime config snapshot (version + effective settings) for client bootstrapping
 * @summary Get runtime bootstrap configuration
 */
export const GetConfigBootstrapResponse = zod.object({
  "version": zod.number().nullable().describe('Global settings version fingerprint (max app_settings.version_number)'),
  "updatedAt": zod.coerce.date().nullable().describe('Timestamp of the newest applied settings change'),
  "settings": zod.object({
  "analyze": zod.object({
  "enabled": zod.boolean(),
  "timeoutMs": zod.number().describe('RunPod analysis request timeout in milliseconds'),
  "version": zod.string(),
  "signedUrlExpiryS": zod.number()
}),
  "upload": zod.object({
  "maxFileSizeBytes": zod.number(),
  "maxDurationS": zod.number(),
  "allowedMimeTypes": zod.array(zod.string()),
  "presignExpiryS": zod.number(),
  "rateLimit": zod.object({
  "max": zod.number(),
  "windowMs": zod.number()
})
}),
  "storage": zod.object({
  "quotaDefaultBytes": zod.number()
}),
  "songs": zod.object({
  "featuredCount": zod.number()
}),
  "learning": zod.object({
  "masteryMinAttempts": zod.number(),
  "masterySuccessRate": zod.number(),
  "demoEnabled": zod.boolean()
}),
  "auth": zod.object({
  "retryAfterSeconds": zod.number()
}),
  "player": zod.object({
  "masterUrlExpiryS": zod.number(),
  "chordLevels": zod.array(zod.string()),
  "speedMin": zod.number(),
  "speedMax": zod.number(),
  "semitonesMin": zod.number(),
  "semitonesMax": zod.number()
}),
  "ui": zod.object({
  "storageWarningThresholds": zod.object({
  "warn": zod.number(),
  "critical": zod.number()
}),
  "postLoginRedirect": zod.string(),
  "urlImportEnabled": zod.boolean(),
  "theme": zod.string().describe('User-app theme variant (\'classic\' or \'new\')'),
  "homeCtaRedirect": zod.string().describe('Landing page primary CTA redirect target')
}),
  "routes": zod.object({
  "musicHubVisible": zod.boolean(),
  "libraryVisible": zod.boolean(),
  "learnVisible": zod.boolean(),
  "processVisible": zod.boolean(),
  "profileVisible": zod.boolean()
}),
  "branding": zod.object({
  "appName": zod.string(),
  "appVersion": zod.string()
})
})
})


/**
 * @summary List all songs in the library
 */
export const ListSongsQueryParams = zod.object({
  "genre": zod.coerce.string().optional(),
  "search": zod.coerce.string().optional()
})

export const ListSongsResponseItem = zod.object({
  "id": zod.number(),
  "title": zod.string(),
  "artist": zod.string().nullable(),
  "artistId": zod.number().nullable(),
  "duration": zod.number().nullable().describe('Duration in seconds'),
  "coverUrl": zod.string().nullable(),
  "bpm": zod.number().nullable(),
  "key": zod.string().nullable(),
  "musicalKey": zod.string().nullish(),
  "mode": zod.string().nullable(),
  "timeSignature": zod.string().nullable(),
  "difficulty": zod.enum(['beginner', 'intermediate', 'advanced']),
  "playCount": zod.number(),
  "featured": zod.boolean(),
  "status": zod.string().optional(),
  "createdAt": zod.coerce.date(),
  "updatedAt": zod.coerce.date()
})
export const ListSongsResponse = zod.array(ListSongsResponseItem)


/**
 * @summary Get public song metadata or an owned private upload
 */
export const GetSongParams = zod.object({
  "id": zod.coerce.number()
})

export const GetSongResponse = zod.object({
  "id": zod.number(),
  "title": zod.string(),
  "artist": zod.string().nullable(),
  "artistId": zod.number().nullable(),
  "duration": zod.number().nullable().describe('Duration in seconds'),
  "coverUrl": zod.string().nullable(),
  "bpm": zod.number().nullable(),
  "key": zod.string().nullable(),
  "musicalKey": zod.string().nullish(),
  "mode": zod.string().nullable(),
  "timeSignature": zod.string().nullable(),
  "difficulty": zod.enum(['beginner', 'intermediate', 'advanced']),
  "playCount": zod.number(),
  "featured": zod.boolean(),
  "status": zod.string().optional(),
  "createdAt": zod.coerce.date(),
  "updatedAt": zod.coerce.date()
})


/**
 * @summary Get featured/trending songs for the landing page
 */
export const GetFeaturedSongsResponseItem = zod.object({
  "id": zod.number(),
  "title": zod.string(),
  "artist": zod.string().nullable(),
  "artistId": zod.number().nullable(),
  "duration": zod.number().nullable().describe('Duration in seconds'),
  "coverUrl": zod.string().nullable(),
  "bpm": zod.number().nullable(),
  "key": zod.string().nullable(),
  "musicalKey": zod.string().nullish(),
  "mode": zod.string().nullable(),
  "timeSignature": zod.string().nullable(),
  "difficulty": zod.enum(['beginner', 'intermediate', 'advanced']),
  "playCount": zod.number(),
  "featured": zod.boolean(),
  "status": zod.string().optional(),
  "createdAt": zod.coerce.date(),
  "updatedAt": zod.coerce.date()
})
export const GetFeaturedSongsResponse = zod.array(GetFeaturedSongsResponseItem)


/**
 * @summary Analyze an owned uploaded song
 */
export const AnalyzeSongParams = zod.object({
  "id": zod.coerce.number()
})

export const AnalyzeSongResponse = zod.object({
  "songId": zod.number(),
  "status": zod.enum(['completed']),
  "beatCount": zod.number(),
  "chordCount": zod.number()
})


/**
 * @summary Get Player aggregate with timelines and signed track URLs
 */
export const GetSongDetailsParams = zod.object({
  "id": zod.coerce.number()
})

export const GetSongDetailsResponse = zod.object({
  "id": zod.number(),
  "title": zod.string(),
  "artist": zod.string(),
  "artistId": zod.number().nullable(),
  "duration": zod.number(),
  "coverUrl": zod.string().nullable(),
  "bpm": zod.number(),
  "key": zod.string(),
  "mode": zod.string().nullable(),
  "difficulty": zod.string(),
  "timeSignature": zod.object({
  "numerator": zod.number(),
  "denominator": zod.number()
}),
  "version": zod.string().nullable(),
  "analysisStatus": zod.string(),
  "generatedAt": zod.coerce.date().nullable(),
  "beatGrid": zod.array(zod.object({
  "time": zod.number(),
  "beat": zod.number(),
  "measure": zod.number(),
  "isDownbeat": zod.boolean()
})),
  "lyrics": zod.array(zod.object({
  "time": zod.number(),
  "text": zod.string(),
  "chords": zod.array(zod.string())
})),
  "chordTimeline": zod.array(zod.object({
  "measure": zod.number(),
  "beat": zod.number(),
  "chord": zod.string(),
  "time": zod.number()
})),
  "sections": zod.array(zod.record(zod.string(), zod.unknown())),
  "tempoTimeline": zod.array(zod.record(zod.string(), zod.unknown())),
  "keyTimeline": zod.array(zod.record(zod.string(), zod.unknown())),
  "tracks": zod.array(zod.object({
  "id": zod.number(),
  "instrument": zod.enum(['master', 'guitar', 'piano', 'bass', 'drums', 'vocal', 'other']),
  "label": zod.string(),
  "volume": zod.number().describe('0-100'),
  "muted": zod.boolean(),
  "soloable": zod.boolean(),
  "pan": zod.number(),
  "streamUrl": zod.string(),
  "offset": zod.number(),
  "normalizationGain": zod.number(),
  "peaks": zod.array(zod.number()).nullable()
})),
  "masterTrackUrl": zod.string().nullable(),
  "playCount": zod.number(),
  "featured": zod.boolean(),
  "createdAt": zod.coerce.date(),
  "updatedAt": zod.coerce.date()
})


/**
 * @summary Get chord library with diagrams
 */
export const ListChordsQueryParams = zod.object({
  "instrument": zod.enum(['guitar', 'piano', 'bass', 'ukulele']).optional(),
  "difficulty": zod.enum(['beginner', 'intermediate', 'advanced']).optional()
})

export const ListChordsResponseItem = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "instrument": zod.string(),
  "difficulty": zod.string(),
  "fingers": zod.array(zod.number()),
  "strings": zod.array(zod.number()),
  "description": zod.string().nullish(),
  "audioUrl": zod.string().nullish()
})
export const ListChordsResponse = zod.array(ListChordsResponseItem)


/**
 * @summary Get chord details and finger positions
 */
export const GetChordParams = zod.object({
  "name": zod.coerce.string()
})

export const GetChordResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "instrument": zod.string(),
  "difficulty": zod.string(),
  "fingers": zod.array(zod.number()),
  "strings": zod.array(zod.number()),
  "description": zod.string().nullish(),
  "audioUrl": zod.string().nullish()
})


/**
 * @summary Get current user profile
 */
export const GetMeResponse = zod.object({
  "id": zod.number(),
  "username": zod.string(),
  "email": zod.string(),
  "avatarUrl": zod.string().nullish(),
  "bio": zod.string().nullish(),
  "preferredInstrument": zod.string().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Update current user profile
 */
export const UpdateMeBody = zod.object({
  "username": zod.string().optional(),
  "bio": zod.string().optional(),
  "preferredInstrument": zod.string().optional()
})

export const UpdateMeResponse = zod.object({
  "id": zod.number(),
  "username": zod.string(),
  "email": zod.string(),
  "avatarUrl": zod.string().nullish(),
  "bio": zod.string().nullish(),
  "preferredInstrument": zod.string().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Get user's storage quota and usage
 */
export const GetStorageQuotaResponse = zod.object({
  "storageUsedBytes": zod.number().describe('Total bytes used by user\'s uploaded files'),
  "storageQuotaBytes": zod.number().describe('Maximum bytes allowed for the user (configurable per-user)')
})


/**
 * @summary Get user's owned uploaded songs
 */
export const GetUserLibraryResponseItem = zod.object({
  "id": zod.number(),
  "title": zod.string(),
  "artist": zod.string().nullable(),
  "artistId": zod.number().nullable(),
  "difficulty": zod.string(),
  "duration": zod.number().nullable(),
  "bpm": zod.number().nullable(),
  "key": zod.string().nullable(),
  "musicalKey": zod.string().nullish(),
  "mode": zod.string().nullable(),
  "timeSignature": zod.string().nullable(),
  "coverUrl": zod.string().nullable(),
  "playCount": zod.number(),
  "featured": zod.boolean(),
  "status": zod.string(),
  "fileKey": zod.string().nullable(),
  "fileUrl": zod.string().nullable(),
  "mimeType": zod.string().nullable(),
  "createdAt": zod.coerce.date(),
  "updatedAt": zod.coerce.date()
})
export const GetUserLibraryResponse = zod.array(GetUserLibraryResponseItem)


/**
 * @summary Permanently delete an owned song and storage objects
 */
export const DeleteLibrarySongParams = zod.object({
  "songId": zod.coerce.number()
})

export const DeleteLibrarySongResponse = zod.object({
  "deleted": zod.boolean()
})


/**
 * @summary Create a quarantine upload URL
 */
export const presignUploadBodyFileNameMax = 255;

export const presignUploadBodyFileSizeMax = 104857600;

export const presignUploadBodyDurationExclusiveMin = 0;
export const presignUploadBodyDurationMax = 600;



export const PresignUploadBody = zod.object({
  "fileName": zod.string().min(1).max(presignUploadBodyFileNameMax),
  "fileSize": zod.number().min(1).max(presignUploadBodyFileSizeMax),
  "mimeType": zod.enum(['audio/mpeg', 'audio/wav', 'audio/flac', 'audio/mp4', 'audio/ogg']),
  "duration": zod.number().gt(presignUploadBodyDurationExclusiveMin).max(presignUploadBodyDurationMax)
})

export const PresignUploadResponse = zod.object({
  "uploadUrl": zod.string().url(),
  "uploadToken": zod.string().uuid(),
  "expiresAt": zod.coerce.date()
})


/**
 * @summary Verify and finalize a quarantine upload
 */
export const confirmUploadBodyExpectedSizeMax = 104857600;

export const confirmUploadBodyTitleMax = 255;

export const confirmUploadBodyDurationExclusiveMin = 0;
export const confirmUploadBodyDurationMax = 600;



export const ConfirmUploadBody = zod.object({
  "uploadToken": zod.string().uuid(),
  "expectedSize": zod.number().min(1).max(confirmUploadBodyExpectedSizeMax),
  "expectedMime": zod.enum(['audio/mpeg', 'audio/wav', 'audio/flac', 'audio/mp4', 'audio/ogg']),
  "title": zod.string().min(1).max(confirmUploadBodyTitleMax).optional(),
  "duration": zod.number().gt(confirmUploadBodyDurationExclusiveMin).max(confirmUploadBodyDurationMax)
})

export const ConfirmUploadResponse = zod.object({
  "songId": zod.number(),
  "status": zod.enum(['uploaded'])
})


/**
 * @summary Cancel an owned quarantine upload
 */
export const CancelUploadParams = zod.object({
  "uploadToken": zod.coerce.string().uuid(),
  "ext": zod.enum(['mp3', 'wav', 'flac', 'm4a', 'ogg'])
})

export const CancelUploadResponse = zod.object({
  "cancelled": zod.boolean()
})


/**
 * @summary Get user's learning sessions
 */
export const GetLearningSessionsResponseItem = zod.object({
  "id": zod.number(),
  "chordName": zod.string(),
  "instrument": zod.string(),
  "startedAt": zod.string(),
  "attemptsCount": zod.number(),
  "successCount": zod.number(),
  "mastered": zod.boolean().optional()
})
export const GetLearningSessionsResponse = zod.array(GetLearningSessionsResponseItem)


/**
 * @summary Start a new learning session for a chord
 */
export const CreateLearningSessionBody = zod.object({
  "chordName": zod.string(),
  "instrument": zod.string()
})


/**
 * @summary Submit a chord attempt result (from mic detection)
 */
export const SubmitChordAttemptParams = zod.object({
  "id": zod.coerce.number()
})

export const SubmitChordAttemptBody = zod.object({
  "success": zod.boolean(),
  "confidenceScore": zod.number().describe('0-1')
})

export const SubmitChordAttemptResponse = zod.object({
  "id": zod.number(),
  "sessionId": zod.number(),
  "success": zod.boolean(),
  "confidenceScore": zod.number(),
  "attemptedAt": zod.string()
})


/**
 * @summary Get list of chords user has mastered
 */
export const GetMasteredChordsResponseItem = zod.object({
  "chordName": zod.string(),
  "instrument": zod.string(),
  "masteredAt": zod.string(),
  "totalAttempts": zod.number(),
  "successRate": zod.number()
})
export const GetMasteredChordsResponse = zod.array(GetMasteredChordsResponseItem)


