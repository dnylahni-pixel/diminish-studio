    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Explore and analyze songs in the catalog.</p>
        </div>
            ...
            <Link key={song.id} href={`/songs/${song.id}`}>
              <div className="group cursor-pointer">
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {Math.floor(song.duration / 60)}:{(song.duration % 60).toString().padStart(2, '0')}</span>
                  <span className="px-2 py-0.5 rounded bg-secondary">{song.key}</span>
                  <span className={`px-2 py-0.5 rounded ${song.difficulty === 'beginner' ? 'bg-green-500/20 text-green-400' : song.difficulty === 'intermediate' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-red-500/20 text-red-400'}`}>
                    {song.difficulty}
                  </span>
                </div>
              </div>
