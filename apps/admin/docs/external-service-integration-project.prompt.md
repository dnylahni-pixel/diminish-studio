# Master Prompt — Documentation-Driven, Merge-Ready Service Integration Project

## How to use this file

Send this entire file to the coding model. Replace only the placeholder below with the official API documentation URL for one service. Use the file once per service (Runpod, Backblaze, Neon, Clerk, Vercel, or Render).

```text
OFFICIAL_DOCUMENTATION_URL = "<PASTE_THE_OFFICIAL_API_DOCUMENTATION_URL_HERE>"
```

The service name and canonical lowercase slug must be inferred from the official documentation and domain. For the six intended runs, canonical slugs are `runpod`, `backblaze`, `neon`, `clerk`, `vercel`, and `render`.
...
Use the `OFFICIAL_DOCUMENTATION_URL` declared at the top of this file. Do not request any private repository path or assume access to the host source tree.
...
/integrations/<service-slug>/<resource>
/integrations/<service-slug>/<resource>/<resource-id>
/integrations/<service-slug>/settings
/integrations/<service-slug>/activity
/integrations/<service-slug>/reports
```

All service-specific API handlers, if genuinely required, live under:

```text
/api/integrations/<service-slug>/...
```
...
src/app/(admin)/integrations/<service-slug>/
  page.tsx
  loading.tsx
...
- use clear service-prefixed names such as `<SERVICE_SLUG>_API_KEY`, adapted to the vendor's official terminology;
...
{
  "schemaVersion": 1,
  "service": "canonical vendor name",
  "serviceSlug": "canonical-lowercase-slug",
  "moduleVersion": "1.0.0",
  ...
  "routePrefix": "/integrations/<service-slug>",
  "apiRoutePrefix": "/api/integrations/<service-slug>",
...
1. Resolve the canonical service name/slug from the official domain.
...
- confirmation that the module owns no route outside `/integrations/<service-slug>` and `/api/integrations/<service-slug>`;
