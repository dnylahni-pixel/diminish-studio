...
```text
TARGET_SERVICE_NAME = "Runpod"
TARGET_SERVICE_SLUG = "runpod"
OFFICIAL_DOCUMENTATION_URL = "https://graphql-spec.runpod.io/#introduction"
```
...
```text
/integrations/runpod
/integrations/runpod/<resource>
/integrations/runpod/<resource>/<resource-id>
/integrations/runpod/settings
/integrations/runpod/activity
/integrations/runpod/reports
```

All service-specific API handlers, if genuinely required, live under:

```text
/api/integrations/runpod/...
```
...
src/app/(admin)/integrations/runpod/
  page.tsx
  loading.tsx
...
- use clear service-prefixed names such as `RUNPOD_API_KEY`, adapted to the vendor's official terminology;
...
```json
{
  ...
  "routePrefix": "/integrations/runpod",
  "apiRoutePrefix": "/api/integrations/runpod",
...
1. Confirm the fixed Runpod identity and `runpod` slug from the official domain.
...
- confirmation that the module owns no route outside `/integrations/runpod` and `/api/integrations/runpod`;
