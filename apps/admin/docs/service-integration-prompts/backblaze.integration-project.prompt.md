...
```text
TARGET_SERVICE_NAME = "Backblaze"
TARGET_SERVICE_SLUG = "backblaze"
OFFICIAL_DOCUMENTATION_URL = "https://www.backblaze.com/docs/cloud-storage-apis"
```
...
/integrations/backblaze/<resource>
/integrations/backblaze/<resource>/<resource-id>
/integrations/backblaze/settings
/integrations/backblaze/activity
/integrations/backblaze/reports
```

All service-specific API handlers, if genuinely required, live under:

```text
/api/integrations/backblaze/...
```
...
src/app/(admin)/integrations/backblaze/
  page.tsx
  loading.tsx
...
- use clear service-prefixed names such as `BACKBLAZE_API_KEY`, adapted to the vendor's official terminology;
...
```json
{
  ...
  "routePrefix": "/integrations/backblaze",
  "apiRoutePrefix": "/api/integrations/backblaze",
...
1. Confirm the fixed Backblaze identity and `backblaze` slug from the official domain.
...
- confirmation that the module owns no route outside `/integrations/backblaze` and `/api/integrations/backblaze`;
