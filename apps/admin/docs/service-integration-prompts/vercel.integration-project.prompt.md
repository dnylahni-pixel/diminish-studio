...
```text
TARGET_SERVICE_NAME = "Vercel"
TARGET_SERVICE_SLUG = "vercel"
...
```text
/integrations/vercel
/integrations/vercel/<resource>
/integrations/vercel/<resource>/<resource-id>
/integrations/vercel/settings
/integrations/vercel/activity
/integrations/vercel/reports
```
...
src/app/(admin)/integrations/vercel/
  page.tsx
  loading.tsx
...
- use clear service-prefixed names such as `VERCEL_API_KEY`, adapted to the vendor's official terminology;
...
```json
{
  ...
  "routePrefix": "/integrations/vercel",
  "apiRoutePrefix": "/api/integrations/vercel",
...
1. Confirm the fixed Vercel identity and `vercel` slug from the official domain.
...
- confirmation that the module owns no route outside `/integrations/vercel` and `/api/integrations/vercel`;
