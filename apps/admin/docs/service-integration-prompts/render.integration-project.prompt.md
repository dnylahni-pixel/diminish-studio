...
```text
TARGET_SERVICE_NAME = "Render"
TARGET_SERVICE_SLUG = "render"
OFFICIAL_DOCUMENTATION_URL = "https://api-docs.render.com/reference/introduction"
```
...
```text
/integrations/render
/integrations/render/<resource>
/integrations/render/<resource>/<resource-id>
/integrations/render/settings
/integrations/render/activity
/integrations/render/reports
```

All service-specific API handlers, if genuinely required, live under:

```text
/api/integrations/render/...
```
...
src/app/(admin)/integrations/render/
  page.tsx
  loading.tsx
...
- use clear service-prefixed names such as `RENDER_API_KEY`, adapted to the vendor's official terminology;
...
```json
{
  ...
  "routePrefix": "/integrations/render",
  "apiRoutePrefix": "/api/integrations/render",
...
1. Confirm the fixed Render identity and `render` slug from the official domain.
...
- confirmation that the module owns no route outside `/integrations/render` and `/api/integrations/render`;
