...
```text
TARGET_SERVICE_NAME = "Neon"
TARGET_SERVICE_SLUG = "neon"
OFFICIAL_DOCUMENTATION_URL = "https://neon.com/docs/reference/api"
```
...
```text
/integrations/neon
/integrations/neon/<resource>
/integrations/neon/<resource>/<resource-id>
/integrations/neon/settings
/integrations/neon/activity
/integrations/neon/reports
```

All service-specific API handlers, if genuinely required, live under:

```text
/api/integrations/neon/...
```
...
src/app/(admin)/integrations/neon/
  page.tsx
  loading.tsx
...
- use clear service-prefixed names such as `NEON_API_KEY`, adapted to the vendor's official terminology;
...
```json
{
  ...
  "routePrefix": "/integrations/neon",
  "apiRoutePrefix": "/api/integrations/neon",
...
1. Confirm the fixed Neon identity and `neon` slug from the official domain.
...
- confirmation that the module owns no route outside `/integrations/neon` and `/api/integrations/neon`;
