"use client";
// Adapted from beui.dev/components/motion/select (token mapping: shadcn roles -> Appica roles; RTL logical props).
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        "hover:border-border-strong focus-visible:ring-2 focus-visible:ring-foreground/20",
...
export function SelectValue({ placeholder, className }: SelectValueProps) {
  ...
  return (
    <span
      className={cn(label ? "text-foreground" : "text-foreground-muted", className)}
    >
...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute start-0 end-0 z-20 rounded-xl border border-border bg-background shadow-lg",
  ...
  return (
    <motion.li variants={ctx.reduce ? undefined : ITEM_VARIANTS}>
      <button
        ...
        className={cn(
          "flex w-full items-center justify-between gap-2 rounded-lg px-2.5 py-1.5 text-start text-sm outline-none transition-colors",
          selected
            ? "bg-background-muted text-foreground"
            : "text-foreground-muted hover:bg-background-muted hover:text-foreground focus-visible:bg-background-muted",
          "disabled:pointer-events-none disabled:opacity-50",
          className,
        )}
      >
