"use client";
// Adapted from beui.dev/components/motion/animated-sidebar
// Token mapping: shadcn roles -> Appica roles; RTL + Next.js navigation added.
...
import { SharedLayoutBg } from "@/components/beui/motion/shared-layout-bg";
import {
  EASE_DRAWER,
  EASE_OUT,
  SPRING_LAYOUT,
...
>(function AnimatedSidebarGroupLabel(
  ...
  return (
    <div
      ...
      className={cn(
        "mb-1 h-7 overflow-hidden px-2 text-[10px] font-medium tracking-[0.14em] text-foreground-muted uppercase transition-opacity",
...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      as="ul"
      inset={0}
      pillClassName="rounded-xl bg-background-muted/70"
    ...
    <AnimatePresence initial={false} mode="popLayout">
      {open && !panel.collapsed ? (
        <motion.ul
          ...
          className={cn(
            "relative mt-1 flex min-w-0 flex-col gap-0.5 border-border ms-5 border-s ps-3",
            className,
          )}
        >
          {children}
...
export function AnimatedSidebarMenuSubButton({
  ...
  const interactiveClassName = cn(
    ...
    "text-foreground-muted transition-colors hover:bg-background-muted/60 hover:text-foreground",
    "focus-visible:bg-background-muted/70 focus-visible:ring-2 focus-visible:ring-ring",
    isActive && "bg-background-muted/70 text-foreground",
...
export function AnimatedSidebarMenuButton({
  ...
  const labelOffset = panel.collapsed ? (direction === "rtl" ? 4 : -4) : 0;
    ...
    <>
      {isActive ? (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl bg-background-muted"
        />
      ) : null}
      {icon ? (
        <span
      ...
      <motion.span
        ...
        animate={{
          ...
          x: panel.collapsed ? labelOffset : 0,
        }}
        transition={
          context.reduce
            ? REDUCED_TRANSITION
      ...
      {badge && !panel.collapsed ? (
        <span className="relative z-10 shrink-0 text-xs text-foreground-muted">
          {badge}
        </span>
      ) : null}
      {ariaExpanded !== undefined ? (
        <motion.span
          ...
          transition={context.reduce ? { duration: 0 } : SPRING_LAYOUT}
          className="relative z-10 grid size-4 shrink-0 place-items-center text-foreground-muted"
        >
          <ChevronRight
            className={cn("size-3.5", direction === "rtl" && "-scale-x-100")}
          />
        </motion.span>
      ) : null}
    </>
