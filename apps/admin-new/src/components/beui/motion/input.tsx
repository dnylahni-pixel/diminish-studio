"use client";
// Adapted from beui.dev/components/motion/input (token mapping: shadcn roles -> Appica roles; RTL logical props).
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "relative h-9 overflow-hidden rounded-lg border transition-colors duration-200",
          "border-border",
          focused && !hasError && "border-foreground/40 ring-2 ring-ring/40",
          hasError && "border-error ring-2 ring-error/25",
          disabled && "opacity-60",
          classNames?.field,
        )}
      >
        {leftIcon ? (
          <span
            className={cn(
              "pointer-events-none absolute start-3 top-1/2 flex -translate-y-1/2 items-center text-foreground-muted [&_svg]:h-4 [&_svg]:w-4",
              classNames?.leftIcon,
            )}
          >
            {leftIcon}
        ...
        {success ? (
          <motion.svg
            ...
            fill="none"
            className={cn(
              "absolute end-3.5 top-1/2 h-5 w-5 -translate-y-1/2 text-success",
          ...
          <span
            className={cn(
              "absolute end-0 top-0 flex h-full items-center text-foreground-muted [&_button]:grid [&_button]:size-9 [&_button]:place-items-center [&_svg]:h-4 [&_svg]:w-4",
