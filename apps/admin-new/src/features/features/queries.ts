...
import { readDefaultAccess } from "./policy";
import type {
  FeatureCreationData,
  FeatureDetailData,
  FeatureEditData,
...
export async function getFeatureEditData(): Promise<FeatureEditData[]> {
  const [rows, dependencyRows] = await Promise.all([
    db
      .select({
        id: features.id,
        code: features.code,
  ...
  for (const row of dependencyRows) {
    const current = dependenciesByFeature.get(row.featureId) ?? [];
    current.push(row.dependsOnFeatureId);
    dependenciesByFeature.set(row.featureId, current);
  }
  ...
  return rows.map((row) => ({
    ...
    defaultAccess: readDefaultAccess(row.metadata) ?? "deny",
    dependencyIds: dependenciesByFeature.get(row.id) ?? [],
  }));
}
