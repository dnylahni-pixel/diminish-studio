...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  const { branding, routes } = useRuntimeConfig();

const navItems = [
  { href: "/music-hub", label: "Music Hub", icon: Music, visible: routes.musicHubVisible },
  { href: "/library", label: "Library", icon: Library, visible: routes.libraryVisible },
  { href: "/learn", label: "Learn", icon: GraduationCap, visible: routes.learnVisible },
  { href: "/process", label: "Process", icon: Upload, visible: routes.processVisible },
  { href: "/profile", label: "Profile", icon: User, visible: routes.profileVisible },
        ...
        <div className={cn(
          ...
          {!collapsed && (
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="font-bold text-sm tracking-tight truncate">{branding.appName}</span>
            </div>
          )}
          {collapsed && (
              ...
              <SheetContent side="left" className="p-0 w-56 border-r-border bg-sidebar">
                <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground p-4 gap-5">
                  <div className="flex items-center gap-2 px-1">
                    ...
                    <span className="font-bold text-sm tracking-tight">{branding.appName}</span>
                  </div>
