/**
 * ATOMIC STORE: uploadStore — isolated copy for __atomic_preview
 * Verbatim mirror of src/stores/uploadStore.ts to keep atomic preview self-contained
 * while preserving real wiring (not mocked). Uses same Zustand shape so it can later
 * be swapped to the global store via import alias without API change.
 */
