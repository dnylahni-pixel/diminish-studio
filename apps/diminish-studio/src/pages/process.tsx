...
const DEFAULT_EXTENSIONS = ['.mp3', '.wav', '.flac', '.m4a', '.ogg'];
...
export function ProcessPage() {
  ...
  // ── Runtime settings (from GET /config/bootstrap; falls back to the old
  ...
  const maxDurationLabel = MAX_DURATION >= 60
    ? `${Math.round(MAX_DURATION / 60)} minutes`
      ...
      <AnimatePresence mode="wait">
        {uploadedSongId ? (
          ...
          >
            ...
            <form onSubmit={handleSubmit} className={`w-full flex gap-3 ${urlImportEnabled ? "" : "opacity-50 pointer-events-none"}`}>
