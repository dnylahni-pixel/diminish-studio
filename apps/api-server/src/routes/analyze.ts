...
router.post("/:id/analyze", async (req, res) => {
  try {
    ...
    // Both gates must pass: the runtime kill-switch AND the boot-time RunPod
    // configuration (secrets are not stored in app_settings).
    if (!runtimeConfig.settings.analyze.enabled || !backendConfig.runPod) {
      return sendError(
        res,
        503,
    ...
    const timeout = setTimeout(
      () => controller.abort(),
      runtimeConfig.settings.analyze.timeoutMs,
    );
